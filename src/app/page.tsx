...
import { navItems } from "@/lib/dduim/data";
...
import { IconBookmark, IconChevronDown, IconMap, IconPlus, IconSearch, IconUser, ShoeGlyph } from "@/components/dduim/icons";
  ...
  return (
    <main className="stage">
      <section className="app-viewport" aria-label="뜀로그 앱">
        ...
        {tab === "saves" && (
          <SavesView courses={courses} pickCourse={pickCourse}
    ...
    <>
      <div style={{ padding: "6px 18px 10px", flexShrink: 0, position: "relative", zIndex: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div className="brand-mark" aria-label="Dduim Log">
            <span className="brand-logo" aria-hidden="true"/>
            <strong>Dduim Log</strong>
          </div>
      ...
      <section className="map-shell">
        <OsmMapView courses={visibleCourses} activeId={activeId}
          ...
          <div className="sheet-content">
            ...
            {orderedCourses.length === 0 && (
              <div className="route-empty-state">
                ...
                <p>지도를 톡톡 찍어서 첫 러닝 코스를 만들어보세요.</p>
                <button type="button" onClick={onStartDrawing}>첫 코스 만들기</button>
              </div>
            )}
          </div>
        </BottomSheet>
...
// ─── SavesView ────────────────────────────────────────────────────────────────
function SavesView({ courses, pickCourse, onEditCourse }: {
  courses: Course[];
  pickCourse: (id: string) => void;
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "10px 18px 0", flexShrink: 0, background: "var(--bg-cream)" }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, letterSpacing: 0, margin: 0 }}>내 코스</h1>
...
// ─── MeView ───────────────────────────────────────────────────────────────────
function MeView({ currentUser, mineCount, totalKm, onRenameUser }: {
  currentUser: { displayName: string; avatarColors: [string, string] };
  mineCount: number; totalKm: number;
  ...
  const handleSave = async () => {
    ...
    if (!trimmed || trimmed === currentUser.displayName) { setEditing(false); return; }
    setSaving(true);
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "10px 18px 110px" }}>
      ...
      {/* 닉네임 수정 */}
      <div style={{ background: "var(--bg-card)", borderRadius: 22, marginTop: 12,
                    border: "1px solid #F2EBDE", overflow: "hidden" }}>
        {editing ? (
          <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-3)" }}>닉네임 변경</div>
            <input
              value={draft}
              ...
              style={{
                ...
                background: "var(--bg-cream)", color: "var(--text-1)", outline: "none", fontWeight: 600,
              }}
            />
          ...
          <button onClick={() => { setDraft(currentUser.displayName); setEditing(true); }} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: "var(--text-3)", marginTop: 2 }}>{currentUser.displayName}</div>
            </div>
      ...
      <p style={{ marginTop: 18, padding: 14, borderRadius: 16,
        ...
        뜀로그 v0.1 · 러닝 코스를 그리고 저장하는 작은 도구
      </p>
    </div>
  );
}
...
function EditCourseSheet({ course, onClose, onSave }: {
  course: Course;
  onClose: () => void;
  onSave: (patch: { title?: string; visibility?: Course["visibility"] }) => Promise<void>;
}) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await onSave({ title: title.trim(), visibility });
    setSaving(false);
  };
